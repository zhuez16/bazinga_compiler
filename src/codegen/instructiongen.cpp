...
std::string adrl(const Reg &target, const Label &source) {
    std::string asm_code;
    asm_code += tab;
    asm_code += "adrl";
    asm_code += " ";
...
std::string bl(const std::string &target_func) {
    std::string asm_code;
    asm_code += tab;
    asm_code += "bl";
    asm_code += " ";
    asm_code += target_func;
    asm_code += newline;
    return asm_code;
}
...
std::string and(const Reg &target, const Reg &op1, const Value &op2) {
    std::string asm_code;
    asm_code += tab;
    asm_code += "and";
    asm_code += " ";
...
std::string ddiv(const Reg &target, const Reg &op1, const Reg &op2) {
    std::string asm_code;
    asm_code += tab;
    asm_code += "ddiv";
    asm_code += " ";
...
std::string bic(const Reg &target, const Reg &v1, const Reg &v2,
    ...
    asm_code += tab + "bic" + " " + vinst_temp_reg.getName() + ", " +
                v1.getName() + ", " + vinst_temp_reg.getName() + ", " + "lsl" +
                " " + v2.getName() + newline;
    asm_code += tab + "orr" + " " + target.getName() + ", " +
